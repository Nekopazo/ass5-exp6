...
[[ -z "${LOG_FILE}" ]] && LOG_FILE="logs/baseline_diffusion_100k_$(date '+%Y%m%d_%H%M%S').log"
[[ -z "${PID_FILE}" ]] && PID_FILE="logs/baseline_diffusion_100k.pid"
...
if [[ "${RUN_MODE}" == "daemon" ]]; then
  ...
  echo "[baseline] pid_file=${ROOT}/${PID_FILE}"
  echo "[baseline] log_file=${ROOT}/${LOG_FILE}"
  exit 0
fi
...
echo "[baseline] root=${ROOT}  pid=$$  device=cuda:0"
...
while true; do
  ...
  if tail -5 "${LOG_FILE}" | grep -q "OutOfMemoryError"; then
    RETRY=$((RETRY + 1))
    echo "[baseline] OOM detected (attempt ${RETRY}), waiting 60s before retry..."
    sleep 60
    echo "[baseline] retrying $(date '+%Y-%m-%d %H:%M:%S')"
  else
    echo "[baseline] non-OOM error (exit code ${RC}), aborting."
    exit $RC
  fi
done

echo "[baseline] done $(date '+%Y-%m-%d %H:%M:%S')"
