...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      exit 0 ;;
    *) echo "[teacher_baseline] unknown arg: $1" >&2; exit 2 ;;
  esac
done
...
SAVE_DIR="checkpoints/teacher_baseline_${RUN_TS}"
if [[ -n "${SAVE_DIR_OVERRIDE}" ]]; then
  SAVE_DIR="${SAVE_DIR_OVERRIDE}"
fi

[[ -z "${LOG_FILE}" ]] && LOG_FILE="logs/teacher_baseline_${RUN_TS}.log"
[[ -z "${PID_FILE}" ]] && PID_FILE="logs/teacher_baseline.pid"
if [[ -n "${RESUME_CKPT}" && ! -f "${RESUME_CKPT}" ]]; then
  echo "[teacher_baseline] resume checkpoint not found: ${RESUME_CKPT}" >&2
  exit 2
fi
...
if [[ "${RUN_MODE}" == "daemon" ]]; then
  ...
  echo "[teacher_baseline] started in background pid=${DAEMON_PID}"
  echo "[teacher_baseline] pid_file=${ROOT}/${PID_FILE}"
  echo "[teacher_baseline] log_file=${ROOT}/${LOG_FILE}"
  exit 0
fi
...
echo "[teacher_baseline] start $(date '+%Y-%m-%d %H:%M:%S')"
echo "[teacher_baseline] root=${ROOT} pid=$$ device=auto save_dir=${SAVE_DIR}"
if [[ -n "${RESUME_CKPT}" ]]; then
  echo "[teacher_baseline] resume_ckpt=${RESUME_CKPT} (will continue from checkpoint step)"
fi
